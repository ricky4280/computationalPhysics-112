...
class Particles:
    ...
    def __init__(self, N:int=0):
        ...
        self._accelerations = np.zeros((N,2))
    ...
    def positions(self, pos: np.ndarray):
        if pos.shape != np.zeros((self.nparticles,2)).shape:
            print("Number of particles does not match!")
            raise ValueError
    ...
    def velocities(self, vel: np.ndarray):
        if vel.shape != np.zeros((self.nparticles,2)).shape:
            print("Number of particles does not match!")
            raise ValueError
    ...
    def accelerations(self, acc: np.ndarray):
        if acc.shape != np.zeros((self.nparticles,2)).shape:
            print("Number of particles does not match!")
            raise ValueError
    ...
    def set_particles(self, pos=None, vel=None, acc=None, tags=None):
        """
        ...
        """
        if pos: self.positions = pos
        if vel: self.velocities = vel
        if acc: self.accelerations = acc
    ...
    def output(self, filename):
        ...
        np.savetxt(filename,(tags[:,0],masses[:,0],pos[:,0],pos[:,1],
                            vel[:,0],vel[:,1],
